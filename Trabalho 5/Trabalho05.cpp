...
typedef LinkedList<Lancamento> ListaContabil;
	...
	while (!sair) {
		...
		case 'r':
			while (true) {
				...
				if (myStream >> valor)
					try {
						listaAtual->remove(valor);
						printf("Lancamento removido com sucesso!\n");
						break;
					} catch (int ex) {
						printf(
								"Número inválido... Entre com um número válido, inteiro, maior ou igual a 0 e menor ou igual a %i !\n",
								listaAtual->count());
					}
			}
			sair = true;
...
void mostraSaldo() {
	...
	for (ListaContabil::iterator it = listaCredito.begin();
			it != listaCredito.end(); ++it)
		tCredito += it->valor();
	for (ListaContabil::iterator it = listaDebito.begin();
			it != listaDebito.end(); ++it)
		tDebito += it->valor();
...
int main() {
	...
	while (!sair) {
		...
		default:
			mensagem =
					"\nOpção inválida selecione uma das opções listadas acima!";
		}
	}
